...
/** How one FIX version's order messages differ from the others (ARCHITECTURE §12). */
export interface DialectRules {
  readonly versionId: string;
  /** HandlInst (21) is required on NewOrderSingle before FIX 4.4. */
  readonly handlInst: boolean;
  /** FIX 4.2 execution reports carry ExecTransType (20)=0. */
  readonly execTransType: boolean;
  /** "F" (Trade) from FIX 4.3; FIX 4.2 reports fills as 1 (partial) or 2 (full). */
  readonly fillExecType: "F" | "PARTIAL_OR_FULL";
}

/** Builds an order dialect from its version rules; everything else is shared. */
export function createDialect(rules: DialectRules): OrderDialect {
  const fillExecType = (status: OrderStatus): string =>
    rules.fillExecType === "F" ? "F" : status === "FILLED" ? "2" : "1";

  return {
    versionId: rules.versionId,

    newOrderSingle(o: NewOrder): FixField[] {
      const fields: FixField[] = [[11, o.clOrdId]];
      if (rules.handlInst) fields.push([21, "1"]);
      fields.push(
        [55, o.symbol],
        [54, SIDE_CODE[o.side]],
        [60, o.transactTime],
    ...
    executionReport(e: ExecEvent): FixField[] {
      const o = e.order;
      const execType = { new: "0", fill: fillExecType(o.status), canceled: "4", rejected: "8" }[
        e.kind
      ];
      ...
      fields.push([17, o.execId]);
      if (rules.execTransType) fields.push([20, "0"]);
      fields.push(
        [150, execType],
