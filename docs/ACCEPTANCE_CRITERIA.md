...
| F-MUST-13 | Version registry, no hard-coded versions | `registry lists every roadmap version as implemented`, `decode rejects unregistered or planned begin strings`, `session uses version profile for logon`, `no hard-coded begin string outside version profiles` |
| F-MUST-14 | Version shown and selectable in UI | `bridge rejects unsupported fix version`, `version picker lists implemented and planned versions`, `switching version reconnects and encodes orders for that version`, `visualizer shows the fix version of each message` |
