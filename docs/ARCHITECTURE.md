...
| Tags, enums, message types | `FixDictionary` per version, built with `defineDictionary(base, overrides)` from the nearest version: 4.3 and 5.0 SP2 extend 4.4, and 4.2 extends 4.3 | 4.2 has ExecTransType (20) and calls tag 32 LastShares; 4.3+ drop 20. ExecType fills are `1`/`2` in 4.2 and `F` in 4.3+. HandlInst (21) is required before 4.4 |
| Session details | `profile.session` (extra Logon fields, admin types). The initiator sends the extra fields; the acceptor requires them (missing → Reject `373=1`, wrong → `373=5`, then Logout) | FIXT Logon adds DefaultApplVerID (1137) |
| Order message shapes | `packages/fix-orders/src/<id>.ts`: `createDialect({ handlInst, execTransType, fillExecType })` returns an `OrderDialect` (isomorphic: the server sends with it and the UI previews with it) | How an ExecutionReport for a fill is expressed |
...
3. Golden vectors for that version in API_CONTRACT §1.4 and `packages/fix-core/test/vectors.ts` (`VERSION_VECTORS`), generated by the engine and verified byte for byte.
4. Tests: `codec round-trips <id> golden vectors`, `session logs on and recovers a gap in <id>`, `exchange flow in <id> produces valid execution reports` (all three loop over `VERSION_VECTORS`, so step 3 adds them), plus `bridge trades over a <id> session`.
5. README and version picker copy (one-line `summary`).

### Version roadmap

| Status | Version | BeginString | Notes |
|---|---|---|---|
| implemented (v1) | FIX 4.4 | `FIX.4.4` | Implemented first; most common in production |
| implemented (M3) | FIX 4.2 | `FIX.4.2` | Still widespread; ExecTransType and old ExecType semantics make a good contrast |
| implemented (M3) | FIX 4.3 | `FIX.4.3` | Bridge between 4.2 and 4.4 |
| implemented (M3) | FIX 5.0 SP2 | `FIXT.1.1` + ApplVerID `9` | Session/application split (FIXT); shows transport independence |
| candidates | FIX 4.0, 4.1, 5.0, 5.0 SP1 | `FIX.4.0`, `FIX.4.1`, `FIXT.1.1` + `7`/`8` | 4.0 has no ResetSeqNumFlag (141), ExecType (150) or LeavesQty (151), so it needs per-version session rules as well as a dialect |

Planned versions are registered with `status: "planned"` (in `versions/planned.ts`), so the picker shows the roadmap, but they can't be selected, and `decode` rejects their BeginString until they're implemented. The roadmap's four versions are all implemented, so the list is currently empty.
